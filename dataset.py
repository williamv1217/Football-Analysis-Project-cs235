...
def dataset(dataset):
	...
	for game in game_ids:
		...
		for i in [1,2]:
			...
			pres_x = [i, home_team_rank, away_team_rank,  count_of_attempt, count_of_corner,count_of_foul, count_of_yellow, count_of_yellow2, count_of_red, count_of_freekick, count_of_offside, count_of_handball, count_of_penalty, count_of_keypass, count_of_failedthroughpass, count_of_ontarget, count_of_offtarget, count_of_blocked, count_of_goalpost]
			if i == 1:
				pres_y = [htgoal]
			else : 
				pres_y = [awgoal]
